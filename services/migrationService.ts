...
import { businessImportPlanner, ImportPlan, ParsedImportWorkbook } from './businessImportPlanner';
...
const classify = (name: string, headers: string[]): ParsedWorksheet['recognizedType'] => {
  ...
  if (sheet.includes('dashboard')) return 'UNKNOWN'; if (sheet.includes('target')) return 'TARGETS'; if (sheet.includes('cmdchain') || sheet.includes('commandchain') || sheet.includes('contact') || sheet.includes('heatmap')) return 'CONTACTS'; if (sheet.includes('worklist') || sheet.includes('engagement') || sheet.includes('task')) return 'WORKLIST'; if (sheet.includes('opp') || sheet.includes('sales') || sheet.includes('referral') || sheet.includes('pipeline')) return 'OPPORTUNITIES'; if (keys.includes('eid') && keys.includes('entity')) return 'TARGETS'; if (keys.includes('pid') || keys.includes('reportstopid')) return 'CONTACTS'; if (keys.includes('engagementdate') || keys.includes('engagementtype')) return 'WORKLIST'; if (keys.includes('estimateddealsize') || keys.includes('osrstatus')) return 'OPPORTUNITIES'; return 'UNKNOWN';
};
const normaliseName = (v: string) => normalizeString(v).toLowerCase();
const userMatch = (name: string, users: UserProfile[]) => { const n = normaliseName(name); if (!n) return null; return users.find((u) => normaliseName(u.displayName) === n) || users.find((u) => normaliseName(u.email) === n) || null; };
...
const engagementTypeFromSource = (value: string): Engagement['engagementType'] => { const v = value.toUpperCase(); if (v.includes('EMAIL')) return 'EMAIL'; if (v.includes('PHONE')) return 'PHONE_CALL'; if (v.includes('SMS')) return 'SMS'; if (v.includes('ONSITE')) return 'MEETING_ONSITE'; if (v.includes('EVENT')) return 'MEETING_EVENT'; if (v.includes('COFFEE')) return 'MEETING_COFFEE'; if (v.includes('LINKEDIN')) return 'LINKEDIN'; if (v.includes('VIDEO')) return 'VIDEO_CONFERENCE'; return 'OTHER'; };
const purposeFromSource = (details: string): Engagement['purpose'] => { const v = details.toUpperCase(); if (v.includes('INTRO')) return 'BUSINESS_INTRODUCTION'; if (v.includes('ESTABLISH')) return 'CONTACT_ESTABLISHMENT'; if (v.includes('MEET')) return 'MEET_AND_GREET'; if (v.includes('REFERR')) return 'REFERRAL'; if (v.includes('DISCOVER')) return 'DISCOVERY'; return 'FOLLOW_UP'; };
...
export const migrationService = {
  ...
  async createImportPlan(workbookResult: WorkbookParseResult): Promise<ImportPlan> {
    ...
    const parsed: ParsedImportWorkbook = {
      workbook: XLSX.utils.book_new(),
      sheets: workbookResult.sheets.map((s) => ({ sheetName: s.sheetName, type: s.recognizedType, rowCount: s.rows.length, headers: s.headers, mappings: [] })),
  ...
  async commitImport(workbookResult: WorkbookParseResult, plan: ImportPlan, user: UserProfile): Promise<MigrationSummaryReport> {
    ...
    const [orgSnap, contactSnap, userSnap, opportunitySnap] = await Promise.all([getDocs(collection(db, 'organisations')), getDocs(collection(db, 'contacts')), getDocs(collection(db, 'users')), getDocs(collection(db, 'opportunities'))]);
    ...
    const existingOpportunities = opportunitySnap.docs.map((d) => ({ id: d.id, ...d.data() })) as Opportunity[];
    const users = userSnap.docs.map((d) => ({ uid: d.id, ...d.data() })) as UserProfile[];
    ...
    for (const { row, type } of allOrgRows) {
      ...
      const existing = similarityMatch(name, existingOrgs); if (existing && existing.confidence >= 95) { orgMap.set(key, existing.org.id); continue; }
      ...
      const org: Organisation = { id: ref.id, name, aliases: [], category, sector: field(row, ['Sector', 'Industry', 'Vertical']) || '', priority: 'MEDIUM', status: 'ACTIVE', assignedBDMId: type === 'TARGETS' ? user.uid : null, location: '', website: '', description: '', notes: type === 'TARGETS' ? '' : 'Imported from operational workbook reference.', lastEngagementDate: null, nextFollowUpDate: null, createdAt: now, createdBy: user.uid, updatedAt: now, updatedBy: user.uid };
    ...
    const contactMap = new Map<string, string>(); const pidMap = new Map<string, string>(); const pendingParents: Array<{ id: string; parentPid: string }> = [];
    ...
    for (const [index, row] of contactRows.entries()) {
      ...
      const first = field(row, ['Fname', 'FirstName', 'GivenName']); const last = field(row, ['Lname', 'LastName', 'Surname']); const full = field(row, ['FullName', 'Name', 'ContactName', 'Stakeholder']) || `${first} ${last}`.trim(); if (!full) { report.validationErrors.push({ entity: 'Contacts', row: index + 4, error: 'Contact name is missing.' }); continue; }
      const parts = full.split(/\s+/); const firstName = first || parts[0]; const lastName = last || parts.slice(1).join(' '); const email = field(row, ['Email', 'EmailAddress', 'WorkEmail']); const pid = field(row, ['PID', 'ContactID', 'ID', 'StakeholderID']);
      ...
      const contactId = duplicate?.id || doc(collection(db, 'contacts')).id;
      if (!duplicate) { const ref = doc(db, 'contacts', contactId); const contact: Contact = { id: contactId, organisationId: orgId, firstName, lastName, fullName: `${firstName} ${lastName}`.trim(), jobTitle: field(row, ['Role', 'JobTitle', 'Title', 'Position']) || 'Stakeholder', department: field(row, ['Department', 'Division', 'Unit', 'Dept']), mobile: field(row, ['Mobile', 'Phone', 'Cell', 'Telephone']), landline: field(row, ['Landline', 'OfficePhone', 'DirectLine']), email, gender: field(row, ['Gender']) || null, reportsToContactId: null, decisionRole: 'UNKNOWN', influenceLevel: 'UNKNOWN', relationshipStrength: 'UNKNOWN', status: 'ACTIVE', notes: '', createdAt: now, createdBy: user.uid, updatedAt: now, updatedBy: user.uid }; addWrite((batch) => batch.set(ref, contact)); report.contactsCreated++; }
      if (pid) pidMap.set(pid, contactId); contactMap.set(`${orgId}|${normaliseName(email || full)}`, contactId); pendingParents.push({ id: contactId, parentPid: field(row, ['ReportsToPID', 'ReportsTo', 'ManagerPID', 'Supervisor']) });
    }
    for (const pending of pendingParents) { if (!pending.parentPid) continue; const parent = pidMap.get(pending.parentPid); if (!parent || parent === pending.id) { report.contactsHierarchyUnresolved++; continue; } if (!existingContacts.some((c) => c.id === pending.id)) addWrite((batch) => batch.update(doc(db, 'contacts', pending.id), { reportsToContactId: parent, updatedAt: now, updatedBy: user.uid })); else report.contactsHierarchyUnresolved++; }
    ...
    for (const [index, row] of workRows.entries()) {
      ...
      const details = field(row, ['EngagementDetails', 'Details', 'Description', 'Notes']); const nextDate = parseDate(field(row, ['NextEngagementDate', 'NextFollowUpDate', 'FollowUpDate'])); const contactName = field(row, ['ContactName', 'ClientContact']); const contactId = contactName ? contactMap.get(`${orgId}|${normaliseName(contactName)}`) || null : null;
      const duplicate = existingOpportunities.length >= 0 && false; // worklist has no stable entity key in the Hub schema; retain every source engagement.
      if (duplicate) continue;
      const engagementRef = doc(collection(db, 'engagements')); const taskRef = doc(collection(db, 'tasks')); const sourceStatus = field(row, ['Status']);
      const engagement: Engagement = { id: engagementRef.id, organisationId: orgId, contactId, assignedTo: user.uid, engagementType: engagementTypeFromSource(field(row, ['EngagementType'])), engagementDate: date, purpose: purposeFromSource(details), details, outcome: field(row, ['Outcome', 'Result']), status: sourceStatus.toUpperCase().includes('HOLD') ? 'ON_HOLD' : 'COMPLETED', engagementCycle: Number(field(row, ['EngagementCycle'])) || null, engagementCycleDescription: field(row, ['EngagementCycleDescr']) || null, nextEngagementDate: nextDate, createdAt: now, createdBy: user.uid, updatedAt: now, updatedBy: user.uid };
    ...
    for (const [index, row] of oppRows.entries()) {
      ...
      const clientContact = field(row, ['ClientContact', 'ContactName']); const contactId = clientContact ? contactMap.get(`${orgId}|${normaliseName(clientContact)}`) || null : null; const rawValue = field(row, ['Estimated Deal Size', 'EstimatedValue', 'DealValue', 'Amount', 'Value']); const value = Number(rawValue.replace(/[^0-9.-]/g, '')); if (Number.isNaN(value)) { report.validationErrors.push({ entity: 'Opportunities', row: index + 4, error: `Estimated deal size is not numeric: ${rawValue}.` }); continue; }
      const sourceId = field(row, ['OSR_ID', 'ID']); const duplicate = existingOpportunities.find((o) => o.organisationId === orgId && normaliseName(o.title) === normaliseName(title)); if (duplicate) continue;
      ...
      addWrite((batch) => batch.set(ref, opportunity)); report.opportunitiesCreated++;
    }
